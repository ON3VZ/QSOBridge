...
function applyHeader(tag, val, s) {
  switch (tag) {
    ...
    case 'LOCATION': s.location = val; break;
...
export function serialize({ qsos, session, profile } = {}) {
  ...
  const catOrder = [
    ['CATEGORY-OPERATOR', cat.operator], ['CATEGORY-ASSISTED', cat.assisted],
    ['CATEGORY-BAND', cat.band], ['CATEGORY-MODE', cat.mode], ['CATEGORY-POWER', cat.power],
    ['CATEGORY-TRANSMITTER', cat.transmitter], ['CATEGORY-TIME', cat.time], ['CATEGORY-OVERLAY', cat.overlay]
  ];
  for (const [k, v] of catOrder) if (v) out.push(`${k}: ${v}`);
  ...
  const wantGrid = profile && profile.header && Array.isArray(profile.header.required) && profile.header.required.includes('myGrid');
  if (s.myGrid && wantGrid) out.push(`GRID-LOCATOR: ${s.myGrid}`);
  if (s.operator || s.stationCall) out.push(`OPERATORS: ${s.operator || s.stationCall}`);
...
function qsoLine(q, s, profile) {
  ...
  if (profile && profile.exchange) {
    sentArr = buildExchange(profile.exchange, 'sent', q, s);
    rcvdArr = buildExchange(profile.exchange, 'rcvd', q, s);
  } else {
    sentArr = [q.rstSent || '', q.serialSent != null ? String(q.serialSent) : (q.exchangeSent.zone || q.exchangeSent.raw || '')].filter((x) => x !== '');
    rcvdArr = [q.rstRcvd || '', q.serialRcvd != null ? String(q.serialRcvd) : (q.exchangeRcvd.zone || q.exchangeRcvd.raw || '')].filter((x) => x !== '');
  }
  const pad = (arr) => arr.map((tok, i) => {
    const t = String(tok);
    if (i === 0) return t.padEnd(2);              // RST
    return /^\d+$/.test(t) ? t.padStart(4) : t.padEnd(6); // volgnr/zone rechts, ref links
  }).join(' ');
  const tag = (q.extras && q.extras.X_QSO) ? 'X-QSO' : 'QSO';
  return `${tag}: ${String(freq).padStart(5)} ${mode.padEnd(2)} ${date} ${time} ${myCall.padEnd(12)} ${pad(sentArr)} ${(q.call || '').padEnd(12)} ${pad(rcvdArr)}`.trimEnd();
}
...
function buildExchange(exchange, who, q, s) {
  ...
  return fields.map((path) => {
    ...
    if (isIotaField && (v == null || v === '' || String(v).toLowerCase() === 'none')) v = '--'; // IOTA-conventie (geen eiland)
