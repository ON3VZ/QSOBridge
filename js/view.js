...
export class App {
  ...
  columns() {
    ...
    if (!prof) return this.ed.orderColumns(base);
    ...
    // Nodige velden vooraan, de rest erna — daarna de gebruikersvolgorde.
    return this.ed.orderColumns([...all.filter((c) => c.required), ...all.filter((c) => !c.required)]);
  }
  ...
  _headerDialog() {
    ...
    const body = `
      ...
      ${row('Locatie (staat/prov of DX)' + need('location'), txt('hLoc', s.location, 90))}
      ...
      ${(prof && prof.id === 'yota') ? row('Leeftijd (YOTA)' + need('myAge'), txt('hAge', s.myAge, 70)) : ''}
      ${H(t('hdr.category'))}
    ...
    dlg.querySelector('#hS').onclick = () => {
      ...
      const loc = dlg.querySelector('#hLoc'); if (loc) s.location = loc.value.trim().toUpperCase() || null;
      ...
      const age = dlg.querySelector('#hAge'); if (age) s.myAge = age.value.trim() ? Number(age.value.replace(/\D/g, '')) : null;
      s.categories = {
  ...
  _colsDialog() {
    const render = () => {
      ...
      const rows = cols.map((c, i) => `<div style="display:flex;align-items:center;gap:8px;padding:3px 0;border-bottom:1px dotted var(--line)">
        <button class="mv" data-k="${c.key}" data-d="-1" ${i === 0 ? 'disabled' : ''} title="omhoog">▲</button>
        <button class="mv" data-k="${c.key}" data-d="1" ${i === cols.length - 1 ? 'disabled' : ''} title="omlaag">▼</button>
        <label style="flex:1"><input type="checkbox" data-c="${c.key}" ${this.ed.hiddenCols.has(c.key) ? '' : 'checked'}> ${c.label}${c.required ? ' <span style="color:var(--amber)">•</span>' : ''}${c.source === 'extra' ? ' <span style="color:var(--muted)">(extra)</span>' : ''}</label>
      </div>`).join('');
      dlg.querySelector('#colsBody').innerHTML = rows;
      ...
      dlg.querySelectorAll('.mv').forEach((b) => b.onclick = (e) => {
        this.ed.moveColumnKey(keys, e.target.dataset.k, Number(e.target.dataset.d));
        this.render(); render();
      });
    };
    const dlg = this._dlg(t('tb.cols'), `<p style="color:var(--muted);font-size:12px;margin:0 0 6px">${t('cols.hint') || 'Vink aan/uit om te tonen; ▲▼ om de kolomvolgorde te wijzigen.'}</p><div id="colsBody"></div>`,
