...
console.log('\n[Fase 5: profielen + validatie]');
{
  ...
  check('ARRL DX: verzendt vermogen, ontvangt staat', /ON3VZ\s+599\s+100\s+K1ABC\s+599\s+MA/.test(arrlLine), `(${arrlLine})`);

  // YOTA: exchange = RST + leeftijd; header met GRID-LOCATOR; CONTEST-tag YOTA
  const yS = makeSession({ stationCall: 'ON3VZ', myGrid: 'JO20EE', myAge: 46 });
  yS.categories = { operator: 'SINGLE-OP', assisted: 'NON-ASSISTED', band: 'ALL', mode: 'MIXED', transmitter: 'ONE' };
  const yQ = makeQso({ call: 'SP2QG', datetime: '2026-07-19T12:39:00Z', band: '15m', freqMHz: 21.279, mode: 'SSB', rstSent: '59', rstRcvd: '59', age: 69 });
  const yOut = cab.serialize({ qsos: [yQ], session: yS, profile: getProfile('yota') }).files[0].content;
  check('YOTA: CONTEST-tag YOTA', /CONTEST: YOTA/.test(yOut));
  check('YOTA: GRID-LOCATOR aanwezig (profiel vraagt het)', /GRID-LOCATOR: JO20EE/.test(yOut));
  check('YOTA: geen CATEGORY-POWER', !/CATEGORY-POWER/.test(yOut));
  check('YOTA: exchange = RST + leeftijd', /ON3VZ\s+59\s+46\s+SP2QG\s+59\s+69/.test(yOut), `(${yOut.split('\n').find((l) => l.startsWith('QSO:'))})`);

  // Kolomvolgorde: een kolom verplaatsen
  const { QsoEditor: EdCol } = await import('../js/grid/editor.js');
  const edCol = new EdCol([makeQso({ call: 'A', band: '20m', mode: 'CW' })], makeSession());
  edCol.moveColumnKey(['call', 'datetime', 'band', 'mode'], 'band', -1);
  check('moveColumnKey verplaatst kolom', edCol.colOrder.slice(0, 3).join(',') === 'call,band,datetime');
...
Fed. Rep. of Germany:     14:  28:  EU:   51.00:   -10.00:    -1.0:  DL:
  ...
  check('IOTA: verzonden ref = -- (geen eiland)', /59\s+1\s+--\s+II3Y/.test(qline), `(${qline})`);
  check('IOTA: ontvangen ref = EU-130', /II3Y\s+59\s+384\s+EU-130/.test(qline), `(${qline})`);
