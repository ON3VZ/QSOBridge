...
- **Editeren:** gevirtualiseerd raster met **dynamische kolommen** — na het inlezen zie je meteen **alle aanwezige velden** (ook alles wat het logprogramma exporteerde). **Kolommen splitsen & samenvoegen** (parse/concat): splits bv. een gecombineerd exchange-veld `599 14` naar RST + CQ-zone (met scheidingsteken óf regex), of voeg serieel + zone samen tot één exchange. Verder: inline bewerken, rij-selectie, filteren, sorteren, kolommen tonen/verbergen, bulk-bewerken, zoek/vervang, undo/redo, rij toevoegen/verwijderen, dupe-detectie, spring-naar-fout.
- **Valideren:** kies een contest/doelformaat en de tool **toont meteen de vereiste velden als kolom** (ook als je log ze nog niet heeft), vooraan en met een amber **verplicht-markering**; ontbrekende/ongeldige verplichte velden krijgen een rode rand, plus een samenvatting "nog nodig: …". Bandplan-consistentie, tooltips en een gezondheidspaneel.
